...
function AddTipoTrabajo() {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:8081/AddTipoTrabajo', values) 
        .then(res => {
            if(res.data.Status === 'Success') { 

                
                axios.post('http://localhost:8081/insertarAuditoria', ['AddTipoTrabajo']) 
                .then(res => { 
                ...
                console.log("Error TipoTrabajo ", error)
            }
        }).catch(err => console.log(err));
    }
    ...
    return (
        ...
        <div className='d-flex justify-content-center align-items-center vh-100 loginPage'>
            <div className='p-3 rounded w-25 border loginForm row align-items-start'>
                ...
                <div>
                    <Link to="/ListaTiposTrabajos" className='btn btn-primary'>Ver tipos de trabajo</Link>
                </div>
            </div>
        </div>
        </>
