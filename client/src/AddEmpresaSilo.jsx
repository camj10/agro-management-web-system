...
function AddEmpresaSilo() {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:8081/AddEmpresaSilo', values) 
        .then(res => { 
            if(res.data.Status === 'Success') { //Acá nos brinda la respuesta si salió bien
                console.log("empresa silo correctamente añadida ",res.data.Status)
                
                
                axios.post('http://localhost:8081/insertarAuditoria', ['AddEmpresaSilo']) 
                .then(res => { 
                    if(res.data.Status === 'Success') { //Acá nos brinda la respuesta si salió bien
    ...
    return (
        ...
        <div className='d-flex justify-content-center align-items-center vh-50 loginPage'>
            <div className='p-3 rounded w-25 border loginForm row align-items-start'>
                ...
                <div>
                    <Link to="/ListaEmpresasSilos" className='btn btn-primary m-1'>Empresas silos</Link>
                </div>
            </div>
        </div>
        </>
